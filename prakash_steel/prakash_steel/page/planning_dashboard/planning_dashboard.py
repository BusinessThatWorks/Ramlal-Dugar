...
def get_sku_type_on_hand_status(filters=None):
	...
	for item in items_with_sku_type:
		...
		print(
			f"[PLANNING DASHBOARD] Item: {item_code} | item_type: '{item_type}' | buffer_flag: '{buffer_flag}' | calculated_sku_type: '{sku_type}'"
		)
	...
	for target_sku in target_sku_types:
		...
		if len(matching_items) <= 50:
			for idx, item in enumerate(matching_items, 1):
				print(
					f"[PLANNING DASHBOARD]   {idx}. {item['item_code']} (item_type: '{item.get('item_type')}', buffer_flag: '{item.get('buffer_flag')}')"
				)
		else:
	...
	for item in items_with_sku_type:
		...
		if sku_type not in target_sku_types:
			if sku_type:  # Only log if SKU type exists (to avoid logging None items)
				print(
					f"[PLANNING DASHBOARD] Skipping {item_code}: SKU type '{sku_type}' not in target list {target_sku_types}"
				)
			continue
		...
		if sku_type == "PTA":
			...
			for colour_name in ["BLACK", "RED", "YELLOW", "GREEN", "WHITE"]:
				if colour_name in colour_data and colour_data[colour_name]["count"] > 0:
					...
					print(
						f"[PLANNING DASHBOARD] PTA {colour_name}: {colour_data[colour_name]['count']} items"
					)
					if len(pta_items) <= 10:
